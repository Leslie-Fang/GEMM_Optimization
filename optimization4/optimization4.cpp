...
void optimization4(shared_ptr<float[]> lhs, shared_ptr<float[]> rhs, shared_ptr<float[]> res, bool verbose){
...
#pragma omp parallel for num_threads(4) private(t0) private(t1) private(res0) private(res1) private(res2) private(res3)
    for(int i=0;i<M;i+=4){
    	for(int j=0;j<N;j+=4){
            res0 = _mm_setzero_ps();
            res1 = _mm_setzero_ps();
    ...
    cout<<"optimization4 Running: "<< (end.tv_sec * 1000 + end.tv_usec / 1000) - (start.tv_sec * 1000 + start.tv_usec / 1000) << "ms"<<endl;
